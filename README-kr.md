...
번역 결과를 받으면 "문장 분석 (Parse Sentence)" 버튼이 나타납니다. LangChain 패키지와 OpenAI의 ChatGPT를 통해 Retrieval-Augmented Generation (RAG) 방식으로 한국어 문장 분석이 가능합니다. 이 과정은 느리고 결과의 정확도가 완벽하지 않지만, 학습자에게 유용한 기능인 것 같습니다. 더 빠른 결과를 받을 수 있도록 외부 패키지로 문장을 분석하고, 문법 해설, 링크 등 추가적인 데이터만 AI로 생성할 생각입니다. 현재는 RAG 방식으로 Chat GPT에게 사용자 원문과 번역 결과, 품사에 대한 자료를 제공하여 분석된 결과를 받습니다. Chat GPT가 문장을 구성하는 단어를 최대한 세분화하여 뜻, 품사 등을 분석하고, 사용자가 쉽게 검색할 수 있도록 사전 검색 링크를 생성합니다. 단어가 외국어일 경우에는 꼬빡꼬빡 링크 대신 Google 검색 링크를 제공합니다. 분석이 완료되면 모든 데이터를 JSON 포맷으로 받습니다.

문장 분석기 아래에서 LangChain과 OpenAI를 이용한 AI 챗봇도 제공됩니다.
